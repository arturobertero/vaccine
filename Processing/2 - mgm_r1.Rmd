...
"Economy prioritized over health",
...
# Main plot as png 
set.seed(100)
p_load(devEMF) # to save as vector

png(
  filename = here("Output", "Article", "MGM_min_06_predictability.png"),
  width = 21.0, 
  height = 29.7, 
  units = "cm",
  res = 300
...
emf(
  file = here("Output", "Article", "MGM_min_06_predictability.emf"),
  width = 8.27, 
  height = 11.69
)
...
# ggplot 
centrality_table <- centralityPlot(
  GraphMGMCENT, 
  include = c("Strength"), 
  scale = "raw",
  orderBy = "Strength", 
...
# png
ggsave(
  filename = here("Output", "Article", "Centrality_Table.png"), 
  plot = centrality_table,
  width = 8, 
  height = 6, 
  units = "in",
  dpi = 300
...
# EMF
ggsave(
  filename = here("Output", "Article", "Centrality_Table.emf"), 
  plot = centrality_table,
  device = devEMF::emf, 
  width = 8, 
  height = 6, 
...
#Merge vectors
degree_data <- data.frame(colnames(W3), degree_cent)
```
...
#summary (uncomment in case you want to see full stats.)
# WARNING: computationally intensive!!
...
```{r}
# 1. Randomly sample half of the row indices
set.seed(1)
sample_indices <- sample(1:nrow(W3), size = floor(nrow(W3) / 2))

# 2. Subset the data using the indices (a gets the sample, b gets the rest)
W3_robustness_a <- W3[sample_indices, ]
W3_robustness_b <- W3[-sample_indices, ]
...
robustness_a_b = NCT(W3_robustness_a, W3_robustness_b, it = 1000,
      ...
      progressbar=T, p.adjust.methods= c("bonferroni"), 
...
# png
ggsave(
  filename = here("Output", "Article", "graph_strength.png"), 
  plot = graph_strength, 
  height = 5, 
  width = 8,
  units = "in",
...
# EMF
ggsave(
  filename = here("Output", "Article", "graph_strength.emf"), 
  plot = graph_strength, 
  device = devEMF::emf, 
  height = 5, 
  width = 8,
...
# extract edge weights connected to hesitancy from the matrix
hesitancy_index <- which(shortnames == "hesitancy")
edges_to_hesitancy <- data.frame(
  names = shortnames,
  edge_weight = inputGraphMGM[, hesitancy_index]
)

# as chat
scatterplot$names <- as.character(scatterplot$names)
...
final_tab <- scatterplot %>%
  ...
  add_row(
    names = "low_educ", 
    coeff_exp = coeff_educ, 
    strength = cent_all$value[which(shortnames == "low_educ")], 
    Significance = ifelse(summary(model_educ)$coeff[2,4] < 0.05, "Significant", "Not significant"), 
    degree = degree_cent[which(shortnames == "low_educ")]
  ) %>%
  # Join the edge weights from the mgm model
  left_join(edges_to_hesitancy, by = "names") %>%
  # Keep ONLY the nodes that actually have a non-zero edge with hesitancy
  filter(edge_weight != 0 & names != "hesitancy") %>%
...
final_tab_z <- final_tab %>% 
  mutate(
    coeff_exp = (coeff_exp - mean(coeff_exp)) / sd(coeff_exp), 
...
# TABLE CENT (Strength & Degree of all nodes for Supplement)

table_cent = cent_all
# Append the raw degree vector directly 
table_cent$Degree = degree_cent
table_cent = table_cent %>% 
  rename(Strength = value) %>%
  dplyr::select(node, Strength, Degree)
