...
```{r}
# ====================================================================
# 1. FINAL TABLE (Empirical Predictors directly tied to Hesitancy)
# ====================================================================

# A. Dynamically extract edge weights connected to hesitancy from the matrix
...
# B. Fix the factor issue so add_row doesn't generate NAs
scatterplot$names <- as.character(scatterplot$names)

# C. Reconstruct the table programmatically
final_tab <- scatterplot %>%
  mutate(degree = scatterplot_degree$degree_cent_graph) %>%
  # Programmatically add the dropped 'low_educ' node back in 
  # We use which() to ensure we match the 17th node regardless of long/short naming
  add_row(
    names = "low_educ", 
  ...
  # Join the exact mathematical edge weights from the mgm model
...
```{r}
# ====================================================================
# 2. TABLE CENT (Strength & Degree of all nodes for Supplement)
# ====================================================================

table_cent = cent_all
# Append the raw degree vector directly (safer than subsetting degree_data)
